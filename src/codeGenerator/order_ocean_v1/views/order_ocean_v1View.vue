...
import { order_ocean_v1Store } from "@/codeGenerator/order_ocean_v1/stores/order_ocean_v1Store";
import order_ocean_v1Service from "@/codeGenerator/order_ocean_v1/services/order_ocean_v1Service.ts";
...
const onSaving = async (e) => {
    e.cancel = true;
    if (e.changes.length) {
        e.promise = await order_ocean_v1Service.ProcessBatchRequest("Default", e.changes, e.component);
    }
};

const loadDefaultOptionGrid = async () => {
gridDataTemplate.value = await order_ocean_v1Store();
